Shader "Texture_Specular_Mask"{
    Properties{
        _MainColor("_MainColor",Color) = (1,1,1,1)
        _MainTex("_MainTex",2D) = "whilte" {}
        _BumpTex("_BumpTex",2D) = "whilte" {}
        _BumpTexIndenty("_BumpTexIndenty",Range(-1,1)) = 0
        _SpeculerMask("_SpeculerMask",2D) = "whilte" {}
        _SpeculerMaskRate("_SpeculerMaskRate",float) = 1.0
    ...
    SubShader{
        pass{
            ...
            float _Gloss;
            sampler2D _SpeculerMask;
            float4 _SpeculerMask_ST;
            float _SpeculerMaskRate;
            ...
            float4 frag(v2f i):SV_TARGET{
                ...
                float speculerMask = tex2D(_SpeculerMask,i.uv.xy).r * _SpeculerMaskRate;
                float3 speculer = _LightColor0.rgb * _SpeculerColor.rgb * pow(saturate(dot(tangentNormal,halfDir)),_Gloss) * speculerMask ;
